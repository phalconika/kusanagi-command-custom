	...
	'help'|'--help')
		case $1 in
			init)
	            ...
	            echo "  "$(eval_gettext "[--ruby|--ruby24]")
				echo "		"$(eval_gettext "Set Ruby Service.")
				exit 0
				;;
			provision)
				...
				echo "	"$(eval_gettext "Option: --rails")
				echo "		"$(eval_gettext "Provision new profile to use Rails.")
				echo "	"$(eval_gettext "Option: --phpmyadmin|--phpMyAdmin")
				echo "		"$(eval_gettext "Provision new profile to use phpMyAdmin.")
				...
				echo "		"$(eval_gettext "If do not specify --email and --no-email, use choose this parameter interactively.")
				echo "	"$(eval_gettext "Option: --dbskip")
				echo "		"$(eval_gettext "skip database, user, password function.")
				...
				echo "		"$(eval_gettext "If do not specify it, use choose this parameter interactively.")
				echo "	"$(eval_gettext "Option: --dst-dir destination_directory")
				echo "		"$(eval_gettext "Set destination directory.")
				exit 0
				;;
			setting)
			...
			ssl)
				...
				echo "	"$(eval_gettext "Option: --hsts [on|off]")
