...
function k_provision {
	...
	local OPT_WPLANG OPT_FQDN OPT_EMAIL OPT_DBNAME OPT_DBUSER OPT_DBPASS OPT_NO_EMAIL OPT_DST_DIR
	local WPLANG= FQDN= MAILADDR= DBNAME= DBUSER= DBPASS= DSTDIR=
	local OPT PRE_OPT
	local APP='WordPress'
	local DBMODE='exec'
	## perse arguments
	...
	do
		...
		elif [ $OPT_DST_DIR ] ; then
			PARENT=`dirname $OPT`
			if [[ -d "$PARENT" ]]; then
				DSTDIR="$OPT"
				OPT_DST_DIR=
			else
				echo $(eval_gettext "option \$PRE_OPT \$PARENT: No Such Directory.")
				return 1
			fi
		else
			case "${OPT}" in
	...
	if [[ ! $NEW_PROFILE =~ ^[a-zA-Z0-9._-]{3,253}$ ]]; then
		echo $(eval_gettext "Failed. Profile name requires [a-zA-Z0-9._-] 3-253 characters.")
		return 1
	fi
	...
	if [ "WordPress" = $APP ]; then
		...
		if ! echo `get_running_db_service` | grep mariadb > /dev/null ; then
			echo $(eval_gettext "You can select only MariaDB for WordPress.")
			echo $(eval_gettext "Use 'kusanagi dbinit mariadb'.")
			return 1
		fi
	...
	if [ "exec" = $DBMODE ]; then
		if ! k_db_provision "$DBNAME" "$DBUSER" "$DBPASS" ; then
			return 1
		fi
	fi
	...
	if [ "$APP" = "Rails" ]; then
		echo $(eval_gettext "Provisioning of NEW_PROFILE completed. Access FQDN") \
				| sed -e "s|NEW_PROFILE|$NEW_PROFILE|" -e "s|FQDN|$FQDN|"
	else
...
# provisioning database for application
function k_db_provision () {
	...
	if get_running_db_service | grep psql > /dev/null ; then
		if ! su - postgres -c "psql --username=postgres --command=\"CREATE ROLE $DBUSER WITH LOGIN ENCRYPTED PASSWORD '$DBPASS'\"" ; then
			return 1
		fi
		if ! su - postgres -c "createdb --encoding=UTF-8 $DBNAME --owner=$DBUSER --username=postgres" ; then
			return 1
		fi
		KUSANAGI_PSQL=yes
	fi
